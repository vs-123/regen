...
size_t calc_fixed_width(const char* regex, size_t len)
{
   size_t size = 0;
   size_t i    = 0;
...
void clear_captures(regen_t* regen, size_t p_idx)
{
...
int match_atom(const char* regex, const char* str, regen_t* regen)
{
   if (*regex == '.') {
      return 1;
   }
...
size_t get_token_len(const char* regex, regen_t* regen)
{
   if (regex[0] == '\\') {
      if (isdigit(regex[1])) {
         return 2;
...
int match_backref(const char* str, size_t str_len, capture_t* cap, int ignore_case)
{
   if (!cap->ptr || str_len < cap->size) {
      return -1;
   }

   int match = 0;
   if (ignore_case) {
      match = (strncasecmp(str, cap->ptr, cap->size) == 0);
   } else {
      match = (memcmp(str, cap->ptr, cap->size) == 0);
   }

   return match ? (int)cap->size : -1;
}

void roll_back_undo_log(regen_t* regen, undo_entry_t* log, size_t count)
{
   while (count > 0) {
      count--;
      regen->captures.elems[log[count].idx] = log[count].old_val;
   }
}

int match_recursive(const char* regex, size_t regex_len, const char* str, size_t str_len,
    const char* orig_str, regen_t* regen, size_t p_idx)
{
   size_t i = 0, j = 0;

   undo_entry_t undo_log[32];
   size_t undo_count = 0;
   ...
   while (i < regex_len) {
      if (regex[i] == '(') {
         ...
         if (type == GROUP_LOOKBEHIND_POS || type == GROUP_LOOKBEHIND_NEG) {
            size_t lb_len = calc_fixed_width(
                regen->groups.elems[target_idx].start, regen->groups.elems[target_idx].size);
            ...
            if ((str + j) - lb_len < orig_str) {
               ...
               consumed
                   = match_alternation((str + j) - lb_len, lb_len, orig_str, regen, target_idx);
            }
         } else {
         ...
         if (type == GROUP_LOOKAHEAD_POS && consumed < 0) {
            roll_back_undo_log(regen, undo_log, undo_count);
            return -1;
         }
         ...
         if (type == GROUP_CAPTURE || type == GROUP_NON_CAPTURE) {
            ...
            if (type == GROUP_CAPTURE && target_idx - 1 < regen->captures.count) {
               if (undo_count < 32) {
                  undo_log[undo_count++] = (undo_entry_t) { .idx = target_idx - 1,
                     .old_val = regen->captures.elems[target_idx - 1] };
               }
      ...
      if (regex[i] == '\\' && isdigit(regex[i + 1])) {
         size_t ref_idx = regex[i + 1] - '0';
         capture_t* cap = &regen->captures.elems[ref_idx - 1];

         if (cap->ptr == NULL || p_idx == ref_idx) {
            roll_back_undo_log(regen, undo_log, undo_count);
            return -1;
         }

         int consumed = match_backref(str + j, str_len - j, cap, regen->is_not_case_sensitive);
         if (consumed < 0) {
            roll_back_undo_log(regen, undo_log, undo_count);
            return -1;
         }
      ...
      if (i + token_len < regex_len
         ...
         if (is_lazy) {
            ...
            for (size_t count = 0; count <= max_matches; count++) {
               if (count >= min_matches) {
                  int res = match_recursive(regex + next_regex_off, regex_len - next_regex_off,
                      str + j_offset + crnt_offset, str_len - (j_offset + crnt_offset), orig_str,
                      regen, p_idx);
                  if (res >= 0) {
               ...
               int consumed = is_group
                   ? match_alternation(str + j_offset + crnt_offset,
                         str_len - (j_offset + crnt_offset), orig_str, regen, target_group_idx)
            ...
            while (count < max_matches) {
               int consumed = is_group
                   ? match_alternation(str + j_offset + crnt_total,
                         str_len - (j_offset + crnt_total), orig_str, regen, target_group_idx)
            ...
            while (count >= min_matches) {
               int res = match_recursive(regex + next_regex_off, regex_len - next_regex_off,
                   str + j_offset + offsets[count], str_len - (j_offset + offsets[count]), orig_str,
                   regen, p_idx);
               if (res >= 0) {
...
int match_alternation(
    const char* str, size_t str_len, const char* orig_str, regen_t* regen, size_t p_idx)
{
...
regen_result_t regen_match(const char* regex, const char* str, regen_t* regen)
{
   ...
   for (size_t i = 0; i < regex_len; i++) {
      ...
      if (regex[i] == '\\' && isdigit(regex[i + 1])) {
         size_t ref = regex[i + 1] - '0';
         if (ref == 0 || ref > regen->captures.count) {
            result.status = REGEN_RES_ERROR_INVALID_BACKREF;
            return result;
         }
      }
   }
